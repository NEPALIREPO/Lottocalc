...
export default function StaffDashboardClient({
  ...
  useEffect(() => {
    const loadUserInfo = async () => {
      ...
      if (user) {
        setUserInfo({ email: user.email || '', name: user.user_metadata?.name });
      }
    };
    loadUserInfo();
  }, [supabase]);
  ...
  const loadActivatedBooks = async () => {
    try {
      ...
      setActivatedBooksList(data as ActivatedBookRow[]);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const handleCreateActivatedBook = async (
    ...
    try {
      ...
      if (created && activatedDate === entryDate) {
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm font-medium text-gray-900">{userInfo?.name || 'Staff'}</p>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Tabs value={activeTab} onValueChange={(v: string) => {
          ...
          <TabsList className="bg-white border border-gray-200 p-1">
            ...
            <TabsTrigger value="players">Player Credit</TabsTrigger>
